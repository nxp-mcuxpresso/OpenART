...
void rt_pin_mux(rt_base_t pin, uint32_t muxRegister,
                          uint32_t muxMode,
...
{
    RT_ASSERT(_hw_pin.ops != RT_NULL);
    _hw_pin.ops->pin_mux(&_hw_pin.parent, muxRegister,muxMode,inputRegister,inputDaisy,configRegister,configValue);
}
FINSH_FUNCTION_EXPORT_ALIAS(rt_pin_mux, pinMux, set hardware pin func);
