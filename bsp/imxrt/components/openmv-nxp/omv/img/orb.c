...
int orb_save_descriptor(FIL *fp, array_t *kpts)
{
    ...
    bytes = dfs_file_write(fp, &kpts_size, sizeof(kpts_size));
    if (bytes != sizeof(kpts_size)) {
        goto error;
    }
    ...
    for (int i=0; i<kpts_size; i++) {
        ...
        // Write X
        bytes = dfs_file_write(fp, &kp->x, sizeof(kp->x));
        if (bytes != sizeof(kp->x)) {
            goto error;
        }

        // Write Y
        bytes = dfs_file_write(fp, &kp->y, sizeof(kp->y));
        if (bytes != sizeof(kp->y)) {
            goto error;
        }

        // Write Score
        bytes = dfs_file_write(fp, &kp->score, sizeof(kp->score));
        if (bytes != sizeof(kp->score)) {
            goto error;
        }

        // Write Octave
        bytes = dfs_file_write(fp, &kp->octave, sizeof(kp->octave));
        if (bytes != sizeof(kp->octave)) {
            goto error;
        }

        // Write Angle
        bytes = dfs_file_write(fp, &kp->angle, sizeof(kp->angle));
        if (bytes != sizeof(kp->angle)) {
            goto error;
        }

        // Write descriptor
        bytes = dfs_file_write(fp, kp->desc, KDESC_SIZE);
        if (bytes != KDESC_SIZE) {
            goto error;
        }
    }
...
int orb_load_descriptor(FIL *fp, array_t *kpts)
{
    ...
    bytes = dfs_file_read(fp, &kpts_size, sizeof(kpts_size));
    if (bytes != sizeof(kpts_size)) {
        goto error;
    }
    ...
    for (int i=0; i<kpts_size; i++) {
        ...
        // Read X
        bytes = dfs_file_read(fp, &kp->x, sizeof(kp->x));
        if (bytes != sizeof(kp->x)) {
            goto error;
        }

        // Read Y
        bytes = dfs_file_read(fp, &kp->y, sizeof(kp->y));
        if (bytes != sizeof(kp->y)) {
            goto error;
        }

        // Read Score
        bytes = dfs_file_read(fp, &kp->score, sizeof(kp->score));
        if (bytes != sizeof(kp->score)) {
            goto error;
        }

        // Read Octave
        bytes = dfs_file_read(fp, &kp->octave, sizeof(kp->octave));
        if (bytes != sizeof(kp->octave)) {
            goto error;
        }

        // Read Angle
        bytes = dfs_file_read(fp, &kp->angle, sizeof(kp->angle));
        if (bytes != sizeof(kp->angle)) {
            goto error;
        }

        // Read descriptor
        bytes = dfs_file_read(fp, kp->desc,  KDESC_SIZE);
        if (bytes != KDESC_SIZE) {
            goto error;
        }
