...
#ifdef MICROPY_HW_ENABLE_SERVO
#define PULSE_TO_DUTY(x) ((x)*10*100*1.0/20000)

pyb_servo_obj_t* s;

extern IrqFuncs_e IRQMAP_Set(IrqSlots_e slot, IrqFuncs_e newFunc);
...
#define Servo_INIT_HAL(n) \
	tmr_base = TMR##n; \
	IRQn = TMR##n##_IRQn; \
     break;
#define Servo_INIT_CH(k) \
	idex = kQTMR_Channel_##k; \
   break;
...
////    pyb_servo_obj_t* s;
////    s =  gc_alloc(sizeof(*s), GC_ALLOC_FLAG_HAS_FINALISER);
////    s->base.type = &pyb_servo_type;
////	  Mux_Take(s, "servo", s->id, "CH", &s->mux);
////  mp_hal_pin_config_alt(s->mux.pPinObj, GPIO_MODE_OUTPUT_PP, AF_FN_TMR);
//   switch(ndx)
//    {
//    case 1:
//        Servo_INIT_HAL(1);
...
//    pyb_servo_obj_t *s = &pyb_servo_obj[0];
...
uint32_t ServoCalcPrescale(int32_t *pQtmrPeriodCnt) {
	...
	#warning "uint32_t clkSource = QTMR_SOURCE_CLOCK;"
	#else
...
void init_servo(pyb_servo_obj_t* s)
{
    ...
    Mux_Take(s, "servo", s->id, "CH", &s->mux);
    mp_hal_pin_config_alt(s->mux.pPinObj, GPIO_MODE_OUTPUT_PP, AF_FN_TMR);
    TMR_Type* tmr_base = TMR1;
    qtmr_channel_selection_t idex = kQTMR_Channel_0;
    IRQn_Type IRQn = TMR1_IRQn;
    int L= strlen(s->mux.szHint);
    const int k = s->mux.szHint[L - 1]- '0';
    const int n = s->mux.szHint[L - 3] - '0';
    switch(n)
    {
    case 1:
        Servo_INIT_HAL(1);
    ...
    s->isActive = true;
    s->pin = s->mux.pPinObj;
    unsigned slot = 4 * (n-1) + k;
    int old = IRQMAP_Set(slot, kIrqFunc_QT_Servo);
...
cleanup:
    ...
    if (self->servo_obj.angle_range == 0)
        self->servo_obj.angle_range = 1;
