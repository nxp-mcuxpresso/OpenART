...
#undef FSL_FEATURE_USDHC_HAS_NO_RW_BURST_LEN
...
#ifdef FSL_FEATURE_USDHC_HAS_NO_RW_BURST_LEN	
...
#else	
	uint32_t pll = CLOCK_GetSysPfdFreq(kCLOCK_Pfd0);
    src_clk = (pll / (CLOCK_GetDiv(mmcsd->usdhc_div) + 1U));
#endif
    //rt_kprintf("\tsrc_clk: %d, usdhc_clk: %d, bus_width: %d\n", src_clk, usdhc_clk, bus_width);
