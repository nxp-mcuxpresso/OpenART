...
{
    gpio_interrupt_mode_t int_mode = kGPIO_IntRisingEdge;;
...
static void imxrt_pin_mux(struct rt_device *device,
						  ...
						  uint32_t configValue)
{
	...
	configValue &= ~(1UL<<31);	// clear MSb, as it is used to mark input/output for GPIO
	
	
	if (muxMode == 0xff) 
		muxMode = REG_READ32(muxRegister) & 7;
	isInputPathForcedOn =  (muxMode == 5 || (configValue | 1<<11) );	// Alt 5 is GPIO or OD mode is selected
	
	CLOCK_EnableClock(kCLOCK_Iomuxc);
	
	IOMUXC_SetPinMux(muxRegister,muxMode,inputRegister,inputDaisy,configRegister,isInputPathForcedOn);
