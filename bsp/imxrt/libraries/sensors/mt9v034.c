...
static int reset(sensor_t *sensor)
{
    ...
    ret |= sensor->cambus_readw(sensor,sensor->slv_addr, MT9V034_CHIP_CONTROL, &chip_control);
    ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_CHIP_CONTROL, (chip_control & (~MT9V034_CHIP_CONTROL_RESERVED)));

    uint16_t read_mode;
    ret |= sensor->cambus_readw(sensor,sensor->slv_addr, MT9V034_READ_MODE, &read_mode);
    ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_READ_MODE, read_mode | MT9V034_READ_MODE_ROW_FLIP | MT9V034_READ_MODE_COL_FLIP);
...
static int read_reg(sensor_t *sensor, uint16_t reg_addr)
{
    ...
    if (sensor->cambus_readw(sensor,sensor->slv_addr, reg_addr, &reg_data) != 0) {
        return -1;
    }
...
{
    return sensor->cambus_writew(sensor,sensor->slv_addr, reg_addr, reg_data);
}
...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    if (sensor->cambus_readw(sensor,sensor->slv_addr, MT9V034_READ_MODE, &read_mode) != 0) {
        return -1;
    }
    ...
    ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_COL_START,
            ((MT9V034_MAX_WIDTH - (width * read_mode_mul)) / 2) + MT9V034_COL_START_MIN);
    ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_ROW_START,
            ((MT9V034_MAX_HEIGHT - (height * read_mode_mul)) / 2) + MT9V034_ROW_START_MIN);
    ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_WINDOW_WIDTH, width * read_mode_mul);
    ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_WINDOW_HEIGHT, height * read_mode_mul);
    ...
    ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_HORIZONTAL_BLANKING,
            MT9V034_HORIZONTAL_BLANKING_DEF + (MT9V034_MAX_WIDTH - IM_MIN(width * read_mode_mul, 640)));

    ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_READ_MODE, read_mode);
    ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_PIXEL_COUNT, (width * height) / 8);

    // We need more setup time for the pixel_clk at the full data rate...
    ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_PIXEL_CLOCK, (read_mode_mul == 1) ? MT9V034_PIXEL_CLOCK_INV_PXL_CLK : 0);
...
{
    uint16_t test;
    int ret = sensor->cambus_readw(sensor,sensor->slv_addr, MT9V034_TEST_PATTERN, &test);
    ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_TEST_PATTERN,
...
{
    uint16_t reg;
    int ret = sensor->cambus_readw(sensor,sensor->slv_addr, MT9V034_AEC_AGC_ENABLE, &reg);
    ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_AEC_AGC_ENABLE,
    ...
    if ((enable == 0) && (!isnanf(gain_db)) && (!isinff(gain_db))) {
        ...
        ret |= sensor->cambus_readw(sensor,sensor->slv_addr, MT9V034_ANALOG_GAIN, &reg);
        ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_ANALOG_GAIN, (reg & 0xFF80) | gain);
    } else if ((enable != 0) && (!isnanf(gain_db_ceiling)) && (!isinff(gain_db_ceiling))) {
        int gain_ceiling = IM_MAX(IM_MIN(fast_roundf(fast_expf((gain_db_ceiling / 20.0) * fast_log(10.0)) * 16.0), 127), 16);

        ret |= sensor->cambus_readw(sensor,sensor->slv_addr, MT9V034_MAX_GAIN, &reg);
        ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_MAX_GAIN, (reg & 0xFF80) | gain_ceiling);
    }
...
{
    uint16_t gain;
    int ret = sensor->cambus_readw(sensor,sensor->slv_addr, MT9V034_ANALOG_GAIN, &gain);
...
static int set_auto_exposure(sensor_t *sensor, int enable, int exposure_us)
{
    ...
    if ((enable == 0) && (exposure_us >= 0)) {
        ret |= sensor->cambus_readw(sensor,sensor->slv_addr, MT9V034_WINDOW_WIDTH, &row_time_0);
        ret |= sensor->cambus_readw(sensor,sensor->slv_addr, MT9V034_HORIZONTAL_BLANKING, &row_time_1);
        ...
        ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_TOTAL_SHUTTER_WIDTH, coarse_time);
        ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_FINE_SHUTTER_WIDTH_TOTAL, fine_time);
        ...
        ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_MAX_EXPOSE, coarse_time);
    }
...
static int get_exposure_us(sensor_t *sensor, int *exposure_us)
{
    ...
    int ret = sensor->cambus_readw(sensor,sensor->slv_addr, MT9V034_TOTAL_SHUTTER_WIDTH, &int_rows);
    ret |= sensor->cambus_readw(sensor,sensor->slv_addr, MT9V034_FINE_SHUTTER_WIDTH_TOTAL, &int_pixels);
    ret |= sensor->cambus_readw(sensor,sensor->slv_addr, MT9V034_WINDOW_WIDTH, &row_time_0);
    ret |= sensor->cambus_readw(sensor,sensor->slv_addr, MT9V034_HORIZONTAL_BLANKING, &row_time_1);
...
{
    uint16_t read_mode;
    int ret = sensor->cambus_readw(sensor,sensor->slv_addr, MT9V034_READ_MODE, &read_mode);
    ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_READ_MODE, // inverted behavior
...
static int ioctl(sensor_t *sensor, int request, va_list ap)
{
    ...
    switch (request) {
        case IOCTL_SET_TRIGGERED_MODE: {
            ...
            ret  = sensor->cambus_readw(sensor,sensor->slv_addr, MT9V034_CHIP_CONTROL, &chip_control);
            ret |= sensor->cambus_writew(sensor,sensor->slv_addr, MT9V034_CHIP_CONTROL,
