...
{
    // Reset all registers
    int ret = sensor->cambus_writeb(sensor,sensor->slv_addr, BANK_SEL, BANK_SEL_SENSOR);
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, COM7, COM7_SRST);
    ...
    for (int i = 0; default_regs[i][0]; i++) {
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, default_regs[i][0], default_regs[i][1]);
    }
...
static int set_pixformat(sensor_t *sensor, pixformat_t pixformat)
{
    ...
    for (int i = 0; regs[i][0]; i++) {
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, regs[i][0], regs[i][1]);
    }
...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, CTRLI, CTRLI_LP_DP | CTRLI_V_DIV_SET(log_div) | CTRLI_H_DIV_SET(log_div));
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, HSIZE, HSIZE_SET(w_mul));
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, VSIZE, VSIZE_SET(h_mul));
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, XOFFL, XOFFL_SET(x_off));
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, YOFFL, YOFFL_SET(y_off));
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, VHYX, VHYX_HSIZE_SET(w_mul) | VHYX_VSIZE_SET(h_mul) | VHYX_XOFF_SET(x_off) | VHYX_YOFF_SET(y_off));
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, TEST, TEST_HSIZE_SET(w_mul));
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, ZMOW, ZMOW_OUTW_SET(w));
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, ZMOH, ZMOH_OUTH_SET(h));
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, ZMHH, ZMHH_OUTW_SET(w) | ZMHH_OUTH_SET(h));
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, R_DVP_SP, div);
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, RESET, 0x00);
...
static int set_contrast(sensor_t *sensor, int level)
{
    ...
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, BANK_SEL, BANK_SEL_DSP);

    /* Write contrast registers */
    for (int i=0; i<sizeof(contrast_regs[0])/sizeof(contrast_regs[0][0]); i++) {
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, contrast_regs[0][i], contrast_regs[level][i]);
    }
...
static int set_brightness(sensor_t *sensor, int level)
{
    ...
    for (int i=0; i<sizeof(brightness_regs[0])/sizeof(brightness_regs[0][0]); i++) {
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, brightness_regs[0][i], brightness_regs[level][i]);
    }
...
static int set_saturation(sensor_t *sensor, int level)
{
    ...
    for (int i=0; i<sizeof(saturation_regs[0])/sizeof(saturation_regs[0][0]); i++) {
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, saturation_regs[0][i], saturation_regs[level][i]);
    }
...
static int set_gainceiling(sensor_t *sensor, gainceiling_t gainceiling)
{
    ...
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, BANK_SEL, BANK_SEL_SENSOR);

    /* Write gain ceiling register */
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, COM9, COM9_AGC_SET(gainceiling));
...
static int set_quality(sensor_t *sensor, int qs)
{
    ...
    /* Write QS register */
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, QS, qs);
...
static int set_auto_gain(sensor_t *sensor, int enable, float gain_db, float gain_db_ceiling)
{
    uint8_t reg;
    int ret = sensor->cambus_writeb(sensor,sensor->slv_addr, BANK_SEL, BANK_SEL_SENSOR);
    ret |= sensor->cambus_readb(sensor,sensor->slv_addr, COM8, &reg);
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, COM8, (reg & (~COM8_AGC_EN)) | ((enable != 0) ? COM8_AGC_EN : 0));
    ...
    if ((enable == 0) && (!isnanf(gain_db)) && (!isinff(gain_db))) {
        ...
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, COM9, (reg & 0x1F) | ((fast_ceilf(fast_log2(gain_ceiling)) - 1) << 5));
    }
...
static int get_gain_db(sensor_t *sensor, float *gain_db)
{
    ...
    // DISABLED
    // if (reg & COM8_AGC_EN) {
    //     ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, COM8, reg & (~COM8_AGC_EN));
    // }
    // DISABLED
...
static int set_auto_exposure(sensor_t *sensor, int enable, int exposure_us)
{
    ...
    if ((enable == 0) && (exposure_us >= 0)) {
        ...
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, BANK_SEL, BANK_SEL_DSP);
        ret |= sensor->cambus_readb(sensor,sensor->slv_addr, IMAGE_MODE, &reg);
        int t_pclk = 1;
        ...
        ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG04, &reg);
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG04, (reg & 0xFC) | ((exposure >> 0) & 0x3));

        ret |= sensor->cambus_readb(sensor,sensor->slv_addr, AEC, &reg);
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, AEC, (reg & 0x00) | ((exposure >> 2) & 0xFF));

        ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG45, &reg);
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG45, (reg & 0xC0) | ((exposure >> 10) & 0x3F));
    }
...
static int get_exposure_us(sensor_t *sensor, int *exposure_us)
{
    ...
    // DISABLED
    // if (reg & COM8_AEC_EN) {
    //     ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, COM8, reg & (~COM8_AEC_EN));
    // }
    // DISABLED

    ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG04, &aec_10);
    ret |= sensor->cambus_readb(sensor,sensor->slv_addr, AEC, &aec_92);
    ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG45, &aec_1510);
...
{
    uint8_t reg;
    int ret = sensor->cambus_writeb(sensor,sensor->slv_addr, BANK_SEL, BANK_SEL_DSP);
    ret |= sensor->cambus_readb(sensor,sensor->slv_addr, CTRL1, &reg);
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, CTRL1, (reg & (~CTRL1_AWB)) | ((enable != 0) ? CTRL1_AWB : 0));
...
static int get_rgb_gain_db(sensor_t *sensor, float *r_gain_db, float *g_gain_db, float *b_gain_db)
{
    ...
    // DISABLED
    // if (reg & CTRL1_AWB) {
    //     ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, CTRL1, reg & (~CTRL1_AWB));
    // }
    // DISABLED
...
static int set_hmirror(sensor_t *sensor, int enable)
{
    ...
    return sensor->cambus_writeb(sensor,sensor->slv_addr, REG04, reg) | ret;
}
...
static int set_special_effect(sensor_t *sensor, sde_t sde)
{
    ...
    switch (sde) {
        case SDE_NEGATIVE:
            ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, BANK_SEL, BANK_SEL_DSP);
            ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, BPADDR, 0x00);
            ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, BPDATA, 0x40);
            ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, BPADDR, 0x05);
            ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, BPDATA, 0x80);
            ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, BPDATA, 0x80);
            break;
        case SDE_NORMAL:
            ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, BANK_SEL, BANK_SEL_DSP);
            ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, BPADDR, 0x00);
            ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, BPDATA, 0x00);
            ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, BPADDR, 0x05);
            ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, BPDATA, 0x80);
            ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, BPDATA, 0x80);
            break;
