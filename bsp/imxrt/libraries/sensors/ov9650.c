...
static int reset(sensor_t *sensor)
{
    ...
    /* Reset all registers */
    sensor->cambus_writeb(sensor,sensor->slv_addr, REG_COM7, 0x80);
...
static int set_pixformat(sensor_t *sensor, pixformat_t pixformat)
{
    ...
    sensor->cambus_readb(sensor,sensor->slv_addr, REG_COM7, &com7);
    ...
    /* Set pixel format */
    sensor->cambus_writeb(sensor,sensor->slv_addr, REG_COM7, com7);
...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    sensor->cambus_writeb(sensor,sensor->slv_addr, REG_COM1, com1);
    sensor->cambus_writeb(sensor,sensor->slv_addr, REG_COM7, com7);
...
static int set_framerate(sensor_t *sensor, framerate_t framerate)
{
    ...
    sensor->cambus_writeb(sensor,sensor->slv_addr, REG_CLKRC, framerate);
    return 0;
}
...
static int set_brightness(sensor_t *sensor, int level)
{
    ...
    for (i=0; i<3; i++) {
        sensor->cambus_writeb(sensor,sensor->slv_addr, regs[0][i], regs[level][i]);
    }
...
static int set_gainceiling(sensor_t *sensor, gainceiling_t gainceiling)
{
    ...
    sensor->cambus_writeb(sensor,sensor->slv_addr, REG_COM9, (gainceiling<<4));
    return 0;
}
...
{
    uint8_t reg;
    int ret = sensor->cambus_readb(sensor,sensor->slv_addr, REG_COM8, &reg);
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG_COM8, (reg & (~REG_COM8_AGC)) | ((enable != 0) ? REG_COM8_AGC : 0));
    ...
    if ((enable == 0) && (!isnanf(gain_db)) && (!isinf(gain_db))) {
        ...
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG_GAIN, ((gain_hi & 0x0F) << 4) | (gain_lo << 0));
        ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG_VREF, &reg);
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG_VREF, ((gain_hi & 0x30) << 2) | (reg & 0x3F));
        ...
        ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG_COM9, &reg);
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG_COM9, (reg & 0x8F) | ((fast_ceilf(fast_log2(gain_ceiling)) - 1) << 4));
    }
...
static int get_gain_db(sensor_t *sensor, float *gain_db)
{
    ...
    // DISABLED
    // if (reg & REG_COM8_AGC) {
    //     ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG_COM8, reg & (~REG_COM8_AGC));
    // }
    // DISABLED

    ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG_GAIN, &gain_lo);
    ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG_VREF, &gain_hi);
...
static int set_auto_exposure(sensor_t *sensor, int enable, int exposure_us)
{
    uint8_t reg;
    int ret = sensor->cambus_readb(sensor,sensor->slv_addr, REG_COM8, &reg);
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG_COM8, (reg & (~REG_COM8_AEC)) | ((enable != 0) ? REG_COM8_AEC : 0));

    if ((enable == 0) && (exposure_us >= 0)) {
        ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG_COM7, &reg);
        int t_line = 1, t_pclk = (reg & REG_COM7_RGB) ? 2 : 1;
        ...
        ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG_CLKRC, &reg);
        ...
        ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG_COM1, &reg);
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG_COM1, (reg & 0xFC) | ((exposure >> 0) & 0x3));

        ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG_AECH, &reg);
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG_AECH, (reg & 0x00) | ((exposure >> 2) & 0xFF));

        ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG_AECHM, &reg);
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG_AECHM, (reg & 0xC0) | ((exposure >> 10) & 0x3F));
    }
...
static int get_exposure_us(sensor_t *sensor, int *exposure_us)
{
    ...
    // DISABLED
    // if (reg & REG_COM8_AEC) {
    //     ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG_COM8, reg & (~REG_COM8_AEC));
    // }
    // DISABLED

    ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG_COM1, &aec_10);
    ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG_AECH, &aec_92);
    ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG_AECHM, &aec_1510);
...
static int set_auto_whitebal(sensor_t *sensor, int enable, float r_gain_db, float g_gain_db, float b_gain_db)
{
    uint8_t reg;
    int ret = sensor->cambus_readb(sensor,sensor->slv_addr, REG_COM8, &reg);
    ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG_COM8, (reg & (~REG_COM8_AWB)) | ((enable != 0) ? REG_COM8_AWB : 0));
    ...
    if ((enable == 0) && (!isnanf(r_gain_db)) && (!isnanf(g_gain_db)) && (!isnanf(b_gain_db))
        ...
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG_BLUE, b_gain);
        ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG_RED, r_gain);
    }
...
static int get_rgb_gain_db(sensor_t *sensor, float *r_gain_db, float *g_gain_db, float *b_gain_db)
{
    ...
    // DISABLED
    // if (reg & REG_COM8_AWB) {
    //     ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG_COM8, reg & (~REG_COM8_AWB));
    // }
    // DISABLED

    ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG_BLUE, &blue);
    ret |= sensor->cambus_readb(sensor,sensor->slv_addr, REG_RED, &red);
...
{
   uint8_t val;
   int ret = sensor->cambus_readb(sensor,sensor->slv_addr, REG_MVFP, &val);
   ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG_MVFP, enable ? (val | REG_MVFP_HMIRROR) : (val & (~REG_MVFP_HMIRROR)));
...
static int set_vflip(sensor_t *sensor, int enable)
{
   uint8_t val;
   int ret = sensor->cambus_readb(sensor,sensor->slv_addr, REG_MVFP, &val);
   ret |= sensor->cambus_writeb(sensor,sensor->slv_addr, REG_MVFP, enable ? (val | REG_MVFP_VFLIP) : (val & (~REG_MVFP_VFLIP)));
