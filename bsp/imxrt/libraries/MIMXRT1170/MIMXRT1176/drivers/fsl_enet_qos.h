...
#define FSL_ENET_QOS_DRIVER_VERSION (MAKE_VERSION(2, 2, 0)) /*!< Version 2.2.0. */
/*@}*/
...
// typedef enum _enet_qos_dma_tx_sche
//{
//    kENET_QOS_FixPri = 0,      /*!< Fixed priority. channel 0 has lower priority than channel 1. */
//    kENET_QOS_WeightStrPri,    /*!< Weighted(burst length) strict priority. */
//    kENET_QOS_WeightRoundRobin /*!< Weighted (weight factor) round robin. */
//} enet_qos_dma_tx_sche;
...
{
    return (base->MAC_MDIO_ADDRESS & ENET_QOS_MAC_MDIO_ADDRESS_GB_MASK) ? true : false;
}
...
{
    return (txDesc->controlStat & ENET_QOS_TXDESCRIP_RD_OWN_MASK) ? true : false;
}
 ...
 * @parame ts     Pointer to the structure @ref enet_qos_ptp_time_t to save frame timestamp.
...
status_t ENET_QOS_SendFrame(
    ENET_QOS_Type *base, enet_qos_handle_t *handle, uint8_t *data, uint32_t length, uint8_t channel, bool isneedTs, void *context);
 ...
 */
void ENET_QOS_PMTIRQHandler(ENET_QOS_Type *base, enet_qos_handle_t *handle);
 ...
 */
void ENET_QOS_Ptp1588StartTimer(ENET_QOS_Type *base, uint32_t ptpClkSrc);
...
{
    uint32_t reg       = 0;
    uint8_t shift      = instance * 8;
    uint8_t pps_config = ENET_QOS_MAC_PPS_CONTROL_TRGTMODSEL0(trgtMode) | ENET_QOS_MAC_PPS_CONTROL_PPSCTRL_PPSCMD(cmd);
    ...
    reg &= ~(0xFF << shift);
    reg |= (pps_config << shift) | ENET_QOS_MAC_PPS_CONTROL_PPSEN0(1);
                                               ...
                                               uint32_t width)
{
    ...
    mac_pps_width = (uint32_t *)((uint8_t *)&base->MAC_PPS0_WIDTH + 0x10 * instance);
                                                  ...
                                                  uint32_t interval)
{
    ...
    mac_pps_interval = (uint32_t *)((uint8_t *)&base->MAC_PPS0_INTERVAL + 0x10 * instance);
