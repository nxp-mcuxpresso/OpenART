...
/*!
 * brief Sets delay value(Including power on delay, power off delay, and so on) of power on/off power domain.
 *
 * param base PGMC basic power controller base address.
...
/*!
 * brief Sets delay value(Including power on delay, power off delay, and so on) of power on/off core's power.
 *
 * param base CPC CORE module base address.
 * param delayOption Pointer to the @ref pgmc_power_switch_delay_option_t structure.
 */
...
/*!
 * brief Sets the assert delay and de-assert delay for the selected signal(Such as Sleep signal) to adjust timing.
 *
 * param base PGMC MIF peripheral base address.
