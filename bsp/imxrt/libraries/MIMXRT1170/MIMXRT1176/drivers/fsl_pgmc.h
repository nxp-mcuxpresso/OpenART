...
/*! @brief The enumeration of PGMC BPC SSAR status flags(Such as Save Busy). */
enum _pgmc_bpc_ssar_status_flags
{
    kPGMC_BPC_DataSavedButNotRestoredFlag = PGMC_BPC_BPC_SSAR_STAT_SAVED_MASK, /*!< Indicate data in this power domain
                                                                                is already saved but not restored yet.
                                                                                */
    kPGMC_BPC_SsarSaveBusyFlag    = PGMC_BPC_BPC_SSAR_STAT_BUSY_SAVE_MASK,     /*!< Busy requesting SSAR save. */
    kPGMC_BPC_SsarRestoreBusyFlag = PGMC_BPC_BPC_SSAR_STAT_BUSY_RESTORE_MASK,  /*!< Busy requesting SSAR restore. */
    kPGMC_BPC_SsarAllFlags =
        kPGMC_BPC_DataSavedButNotRestoredFlag | kPGMC_BPC_SsarSaveBusyFlag | kPGMC_BPC_SsarRestoreBusyFlag,
};
...
/*!
 * @brief PGMC power switch delay structure, including power off/on delay and isolation on/off delay.
 */
...
{
    uint16_t assertDelay;   /*!< Delay before asserting signal to high. */
    uint16_t deAssertDelay; /*!< Delay before de-asserting signal to low. */
} pgmc_mif_signal_delay_t;

/*!
 * @brief PGMC PMIC standby acknowledge configuration, including stand on/off step's count mode and acknowledge count.
 */
 ...
 *                  - \b true Request memory low power level software change in software mode.
 *                  - \b false Do not request memory low power level software change in software mode.
 */
void PGMC_BPC_ControlPowerDomainBySoftwareMode(PGMC_BPC_Type *base, bool powerOff, bool requestMLPLChange);

/*!
 * @brief Sets delay value of power on/off power domain, including power on delay, power off delay, etc.
 *
 * @param base PGMC basic power controller base address.
...
/*!
 * @brief Gets the status flags of SSAR, such as Save Busy status, Restore Busy status, and so on.
 *
 * @param base PGMC basic power controller base address.
 *
 * @return Should be the OR'ed value of @ref _pgmc_bpc_ssar_status_flags.
 */
static inline bool PGMC_BPC_GetSsarStatusFlags(PGMC_BPC_Type *base)
{
    return base->BPC_SSAR_STAT;
}
...
/*!
 * @brief Sets delay value of power on/off core's power, including power on delay, power off delay, etc.
 *
 * @param base CPC CORE module base address.
 * @param delayOption Pointer to the @ref pgmc_power_switch_delay_option_t structure.
 */
...
/*!
 * @brief Sets the assert delay and de-assert delay for the selected signal to adjust timing, such as Sleep signal.
 *
 * @param base PGMC MIF peripheral base address.
...
{
    base->MPC_AUTHEN_CTRL |= PGMC_MIF_MPC_AUTHEN_CTRL_LOCK_CFG_MASK;
}
...
/*!
 * @brief Sets PGMC's PMIC standby module acknowledge config.
 *
 * @param base PMIC module base address.
