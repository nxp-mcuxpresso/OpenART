...
void SSARC_SetDescriptorConfig(SSARC_HP_Type *base, uint32_t index, const ssarc_descriptor_config_t *config)
{
    ...
    base->DESC[index].SRAM0 = config->registerAddress;
    ...
    /* Normally, it is no need to set the value of the register. */
    if (config->registerDataPtr != NULL)
    {
        base->DESC[index].SRAM1 = *config->registerDataPtr;
    }
}
...
void SSARC_GroupInit(SSARC_LP_Type *base, uint8_t groupID, const ssarc_group_config_t *config)
{
    ...
    temp32 = SSARC_LP_DESC_CTRL1_POWER_DOMAIN(config->powerDomain) |
             ...
             SSARC_LP_DESC_CTRL1_RT_PRIORITY(config->restorePriority) | SSARC_LP_DESC_CTRL1_RESD(config->restoreDomain);
