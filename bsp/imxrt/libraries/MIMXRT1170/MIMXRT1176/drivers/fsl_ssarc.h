...
/*! @name Driver version */
/*@{*/
/*! @brief SSARC driver version 2.0.0. */
#define FSL_SSARC_DRIVER_VERSION (MAKE_VERSION(2, 0, 0))
/*@}*/
...
enum _ssarc_interrupt_status_flags
{
    kSSARC_AddressErrorFlag =
        SSARC_LP_INT_STATUS_ADDR_ERR_MASK, /*!< If the descriptor is not in the range, assert address error. */
    kSSARC_AHBErrorFlag =
        SSARC_LP_INT_STATUS_AHB_ERR_MASK, /*!< If any AHB master access receives none-OKAY, assert AHB error. */
    kSSARC_SoftwareRequestDoneFlag = SSARC_LP_INT_STATUS_SW_REQ_DONE_MASK, /*!< If a software triggered save or restore
                                                     process is completed, assert sofware request done . */
...
typedef enum _ssarc_descriptor_operation
{
    ...
    kSSARC_SaveEnableRestoreDisable =
        SSARC_HP_SRAM2_SV_EN_MASK, /*!< Enable Save operation, disable restore operation. */
    kSSARC_SaveDisableRestoreEnable =
        SSARC_HP_SRAM2_RT_EN_MASK, /*!< Disable Save operation, enable restore operation. */
    kSSARC_SaveEnableRestoreEnable =
        (SSARC_HP_SRAM2_RT_EN_MASK |
         SSARC_HP_SRAM2_SV_EN_MASK), /*!< Enable Save operation, enable restore operation. */
...
typedef enum _ssarc_software_trigger_mode
{
    ...
    kSSARC_TriggerRestoreRequest =
        SSARC_LP_DESC_CTRL1_SW_TRIG_RT_MASK, /*!< Trigger the restore operation, don't trigger the save operation. */
} ssarc_software_trigger_mode_t;

/*!
 * @brief The config of descriptor.
 */
typedef struct _ssarc_descriptor_config
{
    ssarc_descriptor_register_size_t size;  /*!< The size of register to be saved/restored. */
    uint32_t registerAddress;               /*!< The address of the register/memory to be saved/restored. */
    uint32_t *registerDataPtr;              /*!< The pointer to the value of the register/memory to be saved/restored.
                                                Normally, it is no need to set the value of the register/memory and
                                                set this parameter as NULL.  */
...
/*!
 * @brief The config of the group.
 */
typedef struct _ssarc_group_config
{
    ssarc_restore_resource_domain_name_t restoreDomain; /*!< Restore resource domain. */
...
/*!
 * @brief De-init the selected group.
 *
 * @param base SSARC_LP peripheral base address.
 * @param groupID The index of the group. Range from 0 to 15.
 */
...
/*!
 * @brief Enable/Disable save/restore request from the GPC module.
 *
 * @param base SSARC_LP peripheral base address.
 * @param enable
