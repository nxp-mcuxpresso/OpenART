...
static uint32_t ASRC_CalculateClockDivider(uint32_t sampleRate_Hz, uint32_t sourceClock_Hz)
{
     ...
     * cycle
     */
    if ((prescaler == 1) && (divider != 1U))
    {
        divider >>= 1U;
        prescaler++;
    }
    /* fine tuning */
    if (sourceClock_Hz / ((1 << prescaler) * divider) > sampleRate_Hz)
    {
        divider++;
    }
                                   ...
                                   uint32_t outSampleRate)
{
    ...
    asrmcra |= ASRC_ASRMCRA_EXTTHRSHA_MASK | ASRC_ASRMCRA_INFIFO_THRESHOLDA(config->inFifoThreshold - 1U) |
               ASRC_ASRMCRA_OUTFIFO_THRESHOLDA(config->outFifoThreshold - 1U);
                                ...
                                uint32_t inSamplesize)
{
    ...
    outSamples = (uint64_t)inSamples * outSampleRate / inSampleRate;
