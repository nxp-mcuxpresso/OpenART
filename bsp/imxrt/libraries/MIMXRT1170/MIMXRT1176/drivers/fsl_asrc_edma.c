...
static void ASRC_OutEDMACallback(edma_handle_t *handle, void *userData, bool done, uint32_t tcds)
{
    ...
    asrcOutHandle->queueDriver                 = (queueDriverIndex + 1) % ASRC_XFER_OUT_QUEUE_SIZE;
...
void ASRC_TransferInCreateHandleEDMA(ASRC_Type *base,
                                     ...
                                     void *userData)
{
    assert(handle && inDmaHandle);

    uint32_t instance = ASRC_GetInstance(base);

    /* Zero the handle */
    memset(&handle->in, 0, sizeof(asrc_in_edma_handle_t));
                                           ...
                                           uint32_t outDataSize)
{
    ...
    handle->out.queueUser                           = (handle->out.queueUser + 1) % ASRC_XFER_OUT_QUEUE_SIZE;
    /* Prepare ASRC output edma configuration */
    EDMA_PrepareTransfer(&config, (void *)outAddr, handle->out.sampleWidth, outDataAddr, handle->out.sampleWidth,
                         handle->out.fifoThreshold * handle->out.sampleWidth, outDataSize, type);

    if (handle->out.state != kStatus_ASRCBusy)
    {
        EDMA_SubmitTransfer(handle->out.outDmaHandle, &config);

        EDMA_StartTransfer(handle->out.outDmaHandle);

        if ((handle->out.peripheralConfig) && (handle->out.peripheralConfig->startPeripheral != NULL))
        {
                                          ...
                                          uint32_t inDataSize)
{
    ...
    edma_transfer_config_t config = {0};
    uint32_t tcdIndex             = handle->in.queueUser;
    ...
    handle->in.queueUser                          = (handle->in.queueUser + 1) % ASRC_XFER_IN_QUEUE_SIZE;
